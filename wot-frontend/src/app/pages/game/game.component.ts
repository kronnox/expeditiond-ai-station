...
export class GameComponent implements OnInit {
  ...
  animate(): void {
    ...
    this.spaceObjects.forEach((spaceObject, indexSO) => {
      ...
      if(spaceObject.projectile != null){
        ...
        if(spaceObject.hit_projectile(spaceObject.projectile)) {
          setTimeout(() => {
            this.spaceObjects.splice(indexSO, 1);
          }, 0)
        }
      } else if (dist < 400 && !spaceObject.detected){
        spaceObject.detect_object();
        if(spaceObject.shoot){
      ...
      if(spaceObject.hit_truck(this.truck)) {
        setTimeout(() => {
          this.spaceObjects.splice(indexSO, 1);
        }, 0)
      }
  ...
  spawnSpaceObjects(){
    setInterval(() => {
      ...
      let img = "";
      if(this.custom_images.length == 0){
        img = this.image_paths[Math.trunc(Math.random()*this.image_paths.length)];
      } else {
        img = this.custom_images[0];
        this.custom_images.splice(0,1);
      }
  ...
  async predictObject(spaceObject: SpaceObject){
    await fetch(spaceObject.image_path).then(r => r.blob()).then(blob => this.backendService.predictBlob(blob)).then(res => {
      const confidence = Math.max(...res);
      spaceObject.predConfidence = confidence.toFixed(2);
      const classId = res.indexOf(confidence);
      spaceObject.shoot = (this.safe.indexOf(classId)==-1);
      spaceObject.predClass = this.backendService.classes[classId];
    });
  }

  saveCanvas(canvas: NgxDrawingCanvasComponent): void {
    this.custom_images.push(canvas.canvas.nativeElement.toDataURL("image/png"));
    canvas.clear();
  }
}
